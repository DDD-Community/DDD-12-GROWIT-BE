...
public class GetMentorAdviceUseCase {
    ...
    public MentorAdvice execute(User user) {
        ...
        final Goal goal = getUserGoalsUseCase.getMyGoals(user, GoalStatus.PROGRESS).stream().findFirst().orElseThrow(() ->
          new NotFoundException("현재 진행중인 목표가 존재하지 않습니다."));
        // 2. 기존 조언이 있는지 확인
        return mentorAdviceRepository.findByUserIdAndGoalId(user.getId(), goal.getId())
                .map(this::handleExistingAdvice).orElse(null);
    }
