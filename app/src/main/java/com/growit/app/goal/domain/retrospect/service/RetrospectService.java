...
public class RetrospectService implements RetrospectValidator {
  ...
  public void validateCreateRetrospect(CreateRetrospectCommand command) throws BadRequestException {
    ...
    if (command.content() == null || command.content().length() < 10 || command.content().length() > 200) {
    ...
    boolean planExists = goal.getPlans().stream()
        .anyMatch(plan -> plan.getId().equals(command.planId()));
    ...
    if (retrospectRepository.findByGoalIdAndPlanId(command.goalId(), command.planId()).isPresent()) {
