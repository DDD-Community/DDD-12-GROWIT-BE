...
public class KakaoOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
  ...
  public OAuth2User loadUser(OAuth2UserRequest req) throws OAuth2AuthenticationException {
    ...
    Optional<User> existing =
        oAuthLinkUseCase.execute(
            new OAuthCommand(profile.getEmail(), profile.getProvider(), profile.getProviderId()));

    if (existing.isPresent()) {
      User user = existing.get();
      attributes.put(JwtClaimKeys.USER, user);
      return new DefaultOAuth2User(
          List.of(new SimpleGrantedAuthority("USER")), attributes, JwtClaimKeys.ID);
    } else {
      ...
      return new DefaultOAuth2User(List.of(), attributes, JwtClaimKeys.ID);
    }
  }
  ...
  public static class KakaoProfile {
    ...
    public static KakaoProfile from(Map<String, Object> attrs) {
      Map<String, Object> account =
          (Map<String, Object>) attrs.getOrDefault(KakaoKeys.KAKAO_ACCOUNT, Map.of());
      Map<String, Object> profile =
          (Map<String, Object>) account.getOrDefault(KakaoKeys.PROFILE, Map.of());
