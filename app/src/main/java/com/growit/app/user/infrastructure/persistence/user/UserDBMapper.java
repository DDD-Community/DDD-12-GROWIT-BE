...
public class UserDBMapper {
  public UserEntity toEntity(User user) {
    return UserEntity.builder().uid(user.getId()).email(user.getEmail().value()).password(user.getPassword()).name(user.getName()).jobRoleId(user.getJobRoleId()).careerYear(user.getCareerYear()).build();
  }
